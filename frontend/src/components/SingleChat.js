...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  const [messages, setMessages] = useState([]); //store the whole chat's data
 ...
 const { user, selectedChat, setSelectedChat,notification,setNotification } = ChatState();
  ...
  const fetchMessages = async () => {
    ...
    try {
      ...
      setMessages(data); // set the currentChat msgs in messages...
      setLoading(false);
  ...
  useEffect(() => {
    socket = io(ENDPOINT); // imported from socketIO client.
  ...
  const typingHandler = (e) => {
    setNewMessage(e.target.value); //set the value of newMessage.
     ...
     try {
      const config={
       ...
       setMessages((msg) =>
          msg.map((itr) =>
        itr._id === messageId
          ? { ...itr, content: "This message is deleted" } // placeholder text
          : itr
      )
    );
          ...
          >
            {loading ? (
              ...
              <div className="messages">
                <ScrollableChat messages={messages} deleteMessage={deleteMessage}/>
